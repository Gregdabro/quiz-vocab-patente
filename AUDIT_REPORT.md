...
| 2026-05-28 | P2.3 — AppHeader inline styles | **DONE** | Все inline `style=` удалены из `AppHeader.jsx`. Стили перенесены в CSS-классы: `.app-header`, `.app-header__container`, `.app-header__left`, `.app-header__title` в `layout.css`. Компонент теперь соответствует архитектурному соглашению. | `src/components/layout/AppHeader.jsx`, `src/styles/layout.css` |
| 2026-05-28 | P3.1 — DictionaryPage double loading | **DONE** | `useVocab` free-режим теперь принимает `null` как сигнал «ещё не готово» (сохраняет `loading=true`). `DictionaryPage` передаёт `null` пока `vocabData` не загружен, вместо пустого массива `[]`. Устранён холостой первый цикл загрузки `useVocab`. | `src/hooks/useVocab.js`, `src/pages/DictionaryPage.jsx` |
| 2026-05-28 | P3.2 — VERIFICA visibility | **DONE** | CSS: добавлена `@keyframes pulse-verifica` — пульсирующая тень на активной кнопке VERIFICA. JS: `finishBtnRef` + `useEffect` — авто-прокрутка к кнопке когда `isFinished` становится `true`. Пользователь видит что тест готов к завершению. | `src/components/quiz/QuizPagination.jsx`, `src/styles/components.css` |
| 2026-05-28 | P3.3 — VocabSessionPage footer delay | **DONE** | Добавлен `showFooter` state с задержкой 1с для empty-сессий (`total===0`). Пользователь успевает прочитать «Все слова изучены» перед появлением кнопки «Продолжить». Normal-сессии (`total>0`) не затронуты — footer появляется сразу. | `src/pages/VocabSessionPage.jsx` |
| 2026-05-28 | P4.1 — filterMasteredWords lemma fix | **DONE** | Убрано `.split(' ')[0]` — теперь используется полная лемма. Многословные фразы (напр. «strada deformata») больше не фильтруются ложно по первому слову. `lemma_topic_map.json` содержит только однословные ключи, поэтому многословные леммы корректно проходят без фильтрации. | `src/services/vocabService.js:262` |
| 2026-05-28 | P4.2 — Vocab-кнопка для completed блоков | **DONE** | В `else`-ветку `BlockCard` (completed/не-current блоки) добавлена кнопка «Слова» рядом с «Повторить». Пользователь может повторить vocab любого пройденного блока. | `src/pages/BlockSelectPage.jsx:317-324` |
| 2026-05-28 | P4.3 — useErrorTopics оптимизация | **DONE** | Заменена загрузка 25 topic-файлов (~12MB) на один `question_topic_map.json` (52KB). Подсчёт ошибок по темам через маппинг `questionId→topicId` — тот же паттерн что в `loadErrorQuestions`. Убран батчинг и `loadTopicQuestions`. | `src/hooks/useErrorTopics.js` |
| 2026-05-28 | P5.1 — localStorage quota guard | **DONE** | Во всех 4 сервисах (`progressService`, `errorsService`, `vocabService`, `blockService`) добавлено различение `QuotaExceededError` с явным предупреждением в консоли. Реальные риски минимальны (~110KB данных при лимите 5-10MB). | `src/services/progressService.js`, `src/services/errorsService.js`, `src/services/vocabService.js`, `src/services/blockService.js` |
| 2026-05-28 | P5.2 — pages.css разбивка | **DONE** | Монолитный `pages.css` (1857 строк) разбит на 7 файлов в `src/styles/pages/`: `home.css`, `quiz.css`, `errors.css`, `stats.css`, `blocks.css`, `vocab-dictionary.css`, `accessibility.css`. Обновлены импорты в `main.jsx` и `index.css`. | `src/styles/pages/*.css`, `src/main.jsx`, `src/index.css` |
| 2026-05-28 | P5.3 — useVocab useRef optimization | **DONE** | `ratedIndices` (useState Set) заменён на `ratedIndicesRef` (useRef) + `ratedCount` (useState number). `rate` callback больше не зависит от `ratedIndices` — ссылка стабильна, меньше пересозданий на iPad mini 2. | `src/hooks/useVocab.js` |
